...
const longestName = (driver, navigator) => {
  ...
  if(driver.length > navigator.length){
    console.log(`The driver ${driver} has the longest name.`);
  } else if(navigator.length > driver.length) {
    console.log(`The navigator ${navigator} has the longest name`)
  } else{
    console.log(`The driver ${driver} and the navigator ${navigator} has equally long names`);
  }
}

longestName("Vanessa", "Julia");
/*
4) 
to get user input you can use the prompt() and then store that into a variable.
Such as 
...
const askUserAge = () => {
  ...
  if(userAge < 15){
    alert(`${userAge}! You are just starting life!`);
  } else if(userAge >= 15 && userAge <= 25){
    alert(`Wow! ${userAge} is a beautiful phase! Enjoy at its max!`);
  } else if(userAge > 25 && userAge <= 35){
    alert(`${userAge}...That's when you still have the humor of a teenager but starts to have the wisdom of an experienced person.`)
  } else{
    alert("After certain age, you will become more and more tired, it's ok.");
  }
}
askUserAge();
...
const calculationTest = () => {
  ...
  */
  ...
  let userAnswer = prompt(
    `What is ${min} * ${max}?
    type your answer (number) below:`
    );
  userAnswer = parseInt(userAnswer);
  const calcAnswer = min * max;
  if(userAnswer === calcAnswer){
    alert(`${userAnswer}! You're right! That's correct! 🥳`);
  } else{
    alert(`💥 No! The answer is ${calcAnswer}`);
  }
};
